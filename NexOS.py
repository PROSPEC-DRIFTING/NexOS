...
def terminal():
    ...
    while True:
        ...
        if command == "help":
            ...
            print("todo           - Terminal task manager")
            print("diagnose       - Run NexOS diagnostics")
            print("sysinfo        - Detailed system information")
            print("base64         - Encode/decode Base64 text")
            print("uuid           - Generate a UUID")
            print("json           - Format and validate JSON")
            print("units          - Unit converter")
            print("weather        - Launch NexOS Weather GUI")
        ...
        # ====================================================
            ...
            print("")
            print("todo")
            print("Terminal task manager with saved tasks.")

            print("")
            print("diagnose")
            print("Run system and dependency diagnostics.")

            print("")
            print("sysinfo")
            print("Detailed Python and platform information.")

            print("")
            print("base64")
            print("Encode or decode Base64 text.")
            ...
            print("")
            print("json")
            print("Format and validate JSON.")

            print("")
            print("units")
            print("Convert common units.")

            print("")
            print("weather")
            print("Launch the NexOS Weather GUI.")
...
def todo_app():
    ...
    while True:
        ...
        print("                   TODO")
        print("=" * 50)
        print("list | add | done | remove | clear | help | exit")
        print("")
        ...
        if command == "list":
            ...
            for i, task in enumerate(tasks, 1):
                ...
                print(f"{i:>3}. [{mark}] {task.get('text', '')}")
            ...
            print("done   - Mark a task complete")
            print("remove - Remove a task")
            print("clear  - Delete all tasks")
            print("exit   - Return to NexOS")

        elif command == "exit":
            break
        else:
            print("Unknown todo command. Type 'help'.")
...
def diagnose():
    ...
    checks.append(("Python", sys.version_info >= (3, 8)))
    checks.append(("JSON system", True))
    checks.append(("Account data", os.path.exists(ACCOUNT_FILE) or True))
    checks.append(("Notes data", os.path.exists(NOTES_FILE) or True))
    checks.append(("Settings data", os.path.exists("settings.json") or True))
    checks.append(("Internet", _internet_check()))
...
def _internet_check():
    try:
        request = urllib.request.Request(
            ...
            headers={"User-Agent": "NexOS-Diagnostics/1.2"}
        )
...
def detailed_sysinfo():
    print("\n" + "=" * 55)
    print("              NEXOS™ SYSTEM INFORMATION")
    print("=" * 55)
    print(f"NexOS version : {VERSION}")
    ...
    while True:
        ...
        if choice == "1":
            ...
            try:
                ...
                print("Invalid Base64 or non-UTF-8 data.")
        elif choice == "3":
            break
        else:
...
def json_tool():
    ...
    print("Paste JSON on one line, or type 'exit'.")
...
def unit_converter():
    ...
    print("5. °C -> °F")
    print("6. °F -> °C")
    print("7. litres -> gallons")
    print("8. gallons -> litres")
    print("9. Exit")

    choice = input("Units> ").strip()
    if choice == "9":
        return
    ...
    conversions = {
        "1": (value * 0.621371, "miles"),
        "2": (value * 1.609344, "km"),
        "3": (value * 2.2046226218, "pounds"),
        "4": (value * 0.45359237, "kg"),
        "5": ((value * 9 / 5) + 32, "°F"),
        "6": ((value - 32) * 5 / 9, "°C"),
        "7": (value * 0.2199692483, "UK gallons"),
        "8": (value * 4.54609, "litres"),
    }

    if choice in conversions:
        result, unit = conversions[choice]
        print(f"Result: {result:.4f} {unit}")
    else:
        print("Invalid option.")


def launch_weather():
    """Launch the separate NexOS Weather GUI if it exists."""
    candidates = [
        "weather.py",
        os.path.join("apps", "weather.py"),
        os.path.join("apps", "weather", "weather.py"),
    ]

    weather_file = next((path for path in candidates if os.path.exists(path)), None)

    if weather_file is None:
        print("Weather app not found.")
        print("Place weather.py beside NexOS.py or inside apps/.")
        return

    try:
        subprocess.Popen([sys.executable, weather_file])
        print("NexOS Weather launched.")
    except Exception as error:
        print(f"Could not launch Weather: {error}")
